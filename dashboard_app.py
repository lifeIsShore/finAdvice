...
def get_model_diagnostics(ticker):
    """Serve detailed diagnostics for analytics page, fallback to consensus"""
    try:
        # 1. Try to find the detailed diagnostics file first (generated by BaselineModels)
        # We look for 1d as the default deep diagnostic
        diag_path = os.path.join(BASE_DIR, "data", "model_diagnostics", f"{ticker}_1d_diagnostics.json")
        
        if os.path.exists(diag_path):
            with open(diag_path, "r", encoding="utf-8") as f:
                return jsonify(json.load(f))
        
        # 2. Fallback: Load the main decision file which contains basic consensus data
        path = os.path.join(DECISIONS_DIR, f"{ticker}_premium_decision.json")
        if not os.path.exists(path):
            return jsonify({"error": f"No diagnostics found for {ticker}. Please run 'Predict' first."}), 404
        
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
            
        # Return the 'consensus' part as a fallback
